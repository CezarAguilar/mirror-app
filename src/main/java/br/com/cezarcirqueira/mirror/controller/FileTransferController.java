...
public class FileTransferController {
    ...
    public ResponseEntity<Void> upload(
        ...
        if (folderPath.isEmpty()) return ResponseEntity.notFound().build();
        try {
    ...
    public ResponseEntity<InputStreamResource> download(
            @RequestParam String guid, @RequestParam String path) {
        Optional<String> folderPath = resolveFolderPath(guid);
        if (folderPath.isEmpty()) return ResponseEntity.notFound().build();
        try {
    ...
    public ResponseEntity<Void> delete(
        ...
        if (folderPath.isEmpty()) return ResponseEntity.notFound().build();
        try {
            Files.deleteIfExists(Paths.get(folderPath.get(),
                    path.replace("/", FileSystems.getDefault().getSeparator())));
            return ResponseEntity.ok().build();
        } catch (Exception e) {
            return ResponseEntity.internalServerError().build();
    ...
    public ResponseEntity<List<Map<String, String>>> syncState(@RequestParam String guid) {
        ...
        try {
            ...
            Files.walk(root).filter(p -> !Files.isDirectory(p)).forEach(p -> {
                ...
                entries.add(Map.of("relativePath", relative, "hash", computeHash(p)));
            });
    ...
    private Optional<String> resolveFolderPath(String guid) {
        if (appStateService.isMaster()) {
            return folderRegistryService.findMasterFolderByGuid(guid).map(f -> f.getFolderPath());
        } else if (appStateService.isSlave()) {
            return folderRegistryService.findMappingByGuid(guid)
                    .filter(m -> m.getLocalPath() != null).map(m -> m.getLocalPath());
        }
